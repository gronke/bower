...
var validateBowerJson = require('../util/validateBowerJson');

function Manager(config, logger, options) {
    this._config = config;
    this._logger = logger;
...
Manager.prototype.preinstall = function (json) {
    ...
    if(!this._options.allowSymlinks) {
        errorWhenSymlink(componentsDir);
    }
...
function errorWhenSymlink(path) {
    if(isSymlink(path)) {
        throw createError(path + ' is a symlink. You can follow symbolic links with the --allow-symlinks flag', 'ESYMLINK');
    }
}
